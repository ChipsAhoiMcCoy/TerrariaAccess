...
/// <summary>
/// Builds tooltip text for items, handling extraction and formatting of tooltip lines.
/// </summary>
internal static class TooltipBuilder
{
    /// <summary>
    /// Builds tooltip details for an item, excluding the item name.
    /// </summary>
    /// <param name="item">The item to describe.</param>
    /// <param name="hoverName">The hover text name (may include prefix).</param>
    /// <param name="allowMouseText">Whether to try extracting from mouse text cache.</param>
    /// <param name="suppressControllerPrompts">Whether to filter out controller prompt lines.</param>
    /// <returns>Formatted tooltip details, or null if none available.</returns>
    public static string? BuildDetails(Item item, string hoverName, bool allowMouseText = true, bool suppressControllerPrompts = false)
    {
    ...
    private static HashSet<string> BuildItemNameCandidates(Item item, string hoverName)
    {
        ...
        AddCandidate(candidates, ItemDescriber.ComposeName(item));
    ...
    {
        return MouseTextCapture.TryGetMouseText();
    }
}
