...
internal abstract class MenuHandlerBase : IMenuHandler
{
    ...
    protected bool TryHandleUiHover(MenuNarrationContext context, List<MenuNarrationEvent> events)
    {
        ...
        ScreenReaderMod.Instance?.Logger.Info($"[MenuHandler] Announcing hover: '{cleaned}'");
    ...
    protected bool TryHandleFocus(MenuNarrationContext context, bool force, List<MenuNarrationEvent> events)
    {
        ...
        if (!FocusResolver.TryGetFocus(context.Main, out MenuFocus focus))
        {
            ...
            {
                ScreenReaderMod.Instance?.Logger.Debug($"[MenuHandler] Unable to determine focus for menu mode {currentMode} (attempt {State.FocusFailureCount}).");
            }
            return false;
        }
        ...
        if (shouldAnnounce)
        {
            ...
            {
                ScreenReaderMod.Instance?.Logger.Info($"[MenuHandler] Focus {focus.Index} via {focus.Source} -> {optionLabel}");
            ...
            {
                ScreenReaderMod.Instance?.Logger.Info($"[MenuHandler] Missing label for focus {focus.Index} (source {focus.Source}) in menu mode {currentMode}.");
        ...
        {
            ScreenReaderMod.Instance?.Logger.Debug($"[MenuHandler] Focus source switched to {focus.Source} for index {focus.Index}.");
        }
