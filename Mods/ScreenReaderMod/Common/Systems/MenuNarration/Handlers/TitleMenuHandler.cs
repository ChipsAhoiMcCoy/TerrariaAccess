...
internal sealed class TitleMenuHandler : MenuHandlerBase
{
    ...
    private bool TryHandleTitleMenuHover(MenuNarrationContext context, List<MenuNarrationEvent> events)
    {
        ...
        {
            ScreenReaderMod.Instance?.Logger.Info($"[TitleMenuHandler] Hover suppressed: {cleaned}");
            return false;
        }

        ScreenReaderMod.Instance?.Logger.Info($"[TitleMenuHandler] Announcing hover: {cleaned}");
