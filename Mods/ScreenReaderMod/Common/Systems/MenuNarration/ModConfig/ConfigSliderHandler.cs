...
internal static class ConfigSliderHandler
{
    ...
    public static bool TryAdjustSlider(UIElement element, int direction, out float newPercentValue)
    {
        ...
        {
            ScreenReaderMod.Instance?.Logger.Debug($"[ConfigSliderHandler] Failed to adjust slider: {ex.Message}");
            return false;
        }
    }
    ...
    private static bool TryCycleEnumElement2(UIElement element, Type type)
    {
        ...
        try
        {
            ...
            ScreenReaderMod.Instance?.Logger.Debug($"[ConfigSliderHandler] Cycling EnumElement2: {currentIndex} -> {newIndex} (max={max})");
        ...
        {
            ScreenReaderMod.Instance?.Logger.Debug($"[ConfigSliderHandler] Failed to cycle EnumElement2: {ex.Message}");
            return false;
        }
    }
    ...
    private static bool TryCycleEnumViaPropotion(UIElement element, Type type)
    {
        ...
        {
            ScreenReaderMod.Instance?.Logger.Debug($"[ConfigSliderHandler] Failed to cycle enum via proportion: {ex.Message}");
            return false;
        }
    }
    ...
    public static bool TryInvokeClick(UIElement element)
    {
        ...
        {
            ScreenReaderMod.Instance?.Logger.Debug($"[ConfigSliderHandler] Failed to invoke click: {ex.Message}");
            return false;
        }
    }
