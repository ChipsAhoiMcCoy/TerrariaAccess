...
internal sealed class TolkSpeechProvider : ISpeechProvider
{
    ...
    public void Initialize()
    {
        ...
        try
        {
            ...
            ScreenReaderMod.Instance?.Logger.Warn($"[Tolk] Unable to locate {TolkLibraryName}. Searched paths: {string.Join(", ", triedPaths)}");
        }
        catch (Exception ex)
        {
            _lastError = ex.Message;
            ScreenReaderMod.Instance?.Logger.Error($"[Tolk] Initialization failed: {ex.Message}");
        }
    }
        ...
        lock (_syncRoot)
        {
            ...
            if (!_available || _speak is null)
            {
                ...
                if (_lastError is not null && !_unavailableLogged)
                {
                    ...
                    ScreenReaderMod.Instance?.Logger.Warn($"[Tolk] Speech unavailable: {_lastError}");
                }
            ...
            try
            {
                ...
                if (!result)
                {
                    ...
                    ScreenReaderMod.Instance?.Logger.Warn("[Tolk] Speak returned false. Disabling speech output until restart.");
                    _available = false;
                }
            }
            catch (Exception ex)
            {
                _lastError = ex.Message;
                ScreenReaderMod.Instance?.Logger.Warn($"[Tolk] Speak threw {ex.Message}. Disabling speech output.");
                _available = false;
            }
        }
    }
    ...
    public void Interrupt()
    {
        lock (_syncRoot)
        {
            ...
            {
                _lastError = ex.Message;
                ScreenReaderMod.Instance?.Logger.Debug($"[Tolk] Silence threw {ex.Message}. Disabling speech output.");
                _available = false;
            }
        }
    }
        ...
        lock (_syncRoot)
        {
            try
            {
                ...
                if (!_hasSpeech())
                {
                    ...
                    ScreenReaderMod.Instance?.Logger.Warn("[Tolk] No screen reader with speech capability detected.");
                    CleanupFailedLoad();
                    return false;
                }
            ...
            {
                _lastError = ex.Message;
                ScreenReaderMod.Instance?.Logger.Debug($"[Tolk] Failed to load {libraryPath}: {ex.Message}");
                CleanupFailedLoad();
                return false;
            }
        }
    ...
    private static IEnumerable<string> EnumerateCandidatePaths()
    {
        ...
        if (!string.IsNullOrWhiteSpace(savePath))
        {
            ...
            yield return Path.Combine(savePath, "ModSources", "ScreenReaderMod", "Libraries", TolkLibraryName);
