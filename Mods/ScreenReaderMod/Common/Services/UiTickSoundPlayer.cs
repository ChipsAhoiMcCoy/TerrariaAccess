...
internal static class UiTickSoundPlayer
{
    ...
    public static void Initialize()
    {
        ...
        {
            global::ScreenReaderMod.ScreenReaderMod.Instance?.Logger.Warn($"[UiTickSoundPlayer] Failed to create tick sound: {ex.Message}");
        }
    }
    ...
    public static void PlaySpatialTick(float pan, float pitch, float volume = 1f)
    {
        ...
        bool spatialEnabled = ScreenReaderModConfig.Instance?.SpatialInventoryAudio ?? true;
        if (!spatialEnabled)
        {
        ...
        try
        {
            ...
            instance.Volume = MathHelper.Clamp(volume * Main.soundVolume, 0f, 1f);
            ...
            {
                instance.Dispose();
                global::ScreenReaderMod.ScreenReaderMod.Instance?.Logger.Debug($"[UiTickSoundPlayer] Play failed: {inner.Message}");
            }
        }
        catch (Exception ex)
        {
            global::ScreenReaderMod.ScreenReaderMod.Instance?.Logger.Warn($"[UiTickSoundPlayer] Tick playback failed: {ex.Message}");
        }
    }
