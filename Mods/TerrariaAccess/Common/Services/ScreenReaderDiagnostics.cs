...
{
    private const string TraceEnvVariable = "TerrariaAccess_TRACE";
    private const string SpeechLogEnvVariable = "TerrariaAccess_SPEECH_LOG_ONLY";
    ...
    internal static void DumpStartupSnapshot(SpeechControllerSnapshot? speechSnapshot = null)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Warn($"[Diagnostics] Failed to gather startup snapshot: {ex.Message}");
        }
    }
    ...
    {
        if (!IsTraceEnabled() || TerrariaAccess.Instance?.Logger is not { } logger)
        {
            return;
        }
    ...
    {
        if (TerrariaAccess.Instance?.Logger is not { } logger || Main.instance is null)
        {
            return;
        }
    ...
    {
        if (_langSnapshotPrinted || TerrariaAccess.Instance?.Logger is not { } logger)
        {
            return;
        }
