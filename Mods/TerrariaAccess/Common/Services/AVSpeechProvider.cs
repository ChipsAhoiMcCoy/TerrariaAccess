...
internal sealed class AvFoundationSpeechProvider : ISpeechProvider
{
        ...
        lock (_syncRoot)
        {
            ...
            try
            {
                ...
                if (_synthesizer == IntPtr.Zero)
                {
                    ...
                    TerrariaAccess.Instance?.Logger.Warn("[AVFoundation] Failed to create AVSpeechSynthesizer.");
                    Unload();
                    return;
                }
                ...
                TerrariaAccess.Instance?.Logger.Info($"[AVFoundation] Connected using AVSpeechSynthesizer (provider: {_providerNumber}).");
            }
            catch (Exception ex)
            {
                _lastError = ex.Message;
                TerrariaAccess.Instance?.Logger.Error($"[AVFoundation] Initialization failed: {ex.Message}");
                Unload();
            }
        }
    }
    ...
    public void Speak(string message)
    {
        lock (_syncRoot)
        {
            ...
            {
                _lastError = ex.Message;
                TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] Speak failed: {ex.Message}");
            }
        }
    }
    ...
    public void Interrupt()
    {
        lock (_syncRoot)
        {
            ...
            {
                _lastError = ex.Message;
                TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] Interrupt failed: {ex.Message}");
            }
        }
    }
    ...
    private void DisposeSynthesizer()
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] Release failed: {ex.Message}");
        }
        finally
        {
    ...
    public void SetRate(float rate)
    {
        lock (_syncRoot)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] SetRate failed: {ex.Message}");
            }
        }
    }
    ...
    public void SetVolume(float volume)
    {
        lock (_syncRoot)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] SetVolume failed: {ex.Message}");
            }
        }
    }
    ...
    public void SetVoice(string voiceIdentifier)
    {
        lock (_syncRoot)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] SetVoice failed: {ex.Message}");
            }
        }
    }
        ...
        foreach (string libraryPath in EnumerateCandidatePaths())
        {
            try
            {
                if (NativeLibrary.TryLoad(libraryPath, out _libraryHandle))
                {
                    ...
                    TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] Successfully loaded {libraryPath}.");
                    return true;
                }
            }
            catch (Exception ex)
            {
                TerrariaAccess.Instance?.Logger.Debug($"[AVFoundation] Failed to load {libraryPath}: {ex.Message}");
            }
        }

        _lastError = $"Failed to load native library '{LibraryName}' using candidate paths.";
        TerrariaAccess.Instance?.Logger.Warn($"[AVFoundation] Failed to load native library '{LibraryName}' from any candidate path.");
        _available = false;
        return false;
    }
    ...
    private static IEnumerable<string> EnumerateCandidatePaths()
    {
        ...
        {
            // 3. Path in ModSources/TerrariaAccess/Libraries (for development)
            yield return Path.Combine(savePath, "ModSources", "TerrariaAccess", "Libraries", LibraryName);
            
            // 4. Path in Mods/TerrariaAccess/Content/Libraries (for built/installed mod)
            yield return Path.Combine(savePath, "Mods", "TerrariaAccess", "Libraries", LibraryName);
            
            // 5. Simpler paths for broader searching
            yield return Path.Combine(savePath, "Mods", LibraryName);
            yield return Path.Combine(savePath, "ModSources", "TerrariaAccess", LibraryName);
