...
internal sealed class MultiplayerFootstepEmitter : AudioEmitterBase
{
    ...
    public override void Update(Player player)
    {
        ...
        bool enabled = TerrariaAccessConfig.Instance?.MultiplayerFootstepsEnabled ?? true;
        if (!enabled)
        {
            Reset();
            return;
        ...
        for (int i = 0; i < Main.maxPlayers; i++)
        {
            ...
            Player remotePlayer = Main.player[i];
            if (!CanProcessRemotePlayer(remotePlayer))
            {
                continue;
            }

            activePlayers.Add(i);
            UpdatePlayerFootsteps(player, remotePlayer, i);
        }
    ...
    private static bool CanProcessRemotePlayer(Player player)
    {
