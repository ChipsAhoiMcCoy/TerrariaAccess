...
public sealed partial class InGameNarrationSystem
{
    private sealed class IngameSettingsNarrator
    {
        ...
        private static string TryGetLanguageValue(string key)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] Language key '{key}' lookup failed: {ex.Message}");
            }
        ...
        {
            if (TerrariaAccess.Instance is null)
            {
                return;
            }
        ...
        private void EnsureReflection()
        {
            ...
            try
            {
                ...
                if (!_loggedFieldCatalog)
                {
                    ...
                    {
                        TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] field: {field.FieldType.FullName} {field.Name}");
                    }

                    foreach (PropertyInfo property in optionsType.GetProperties(flags).OrderBy(p => p.Name))
                    {
                        TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] property: {property.PropertyType.FullName} {property.Name}");
                    }
                ...
                TerrariaAccess.Instance?.Logger.Debug("[IngameSettings] Reflection resolved: " +
            ...
            {
                TerrariaAccess.Instance?.Logger.Warn($"[IngameSettings] Reflection resolution failed: {ex.Message}");
                _fieldsResolved = true;
            }
        }
        ...
        private static int ReadInt(FieldInfo? field)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] Failed to read {field?.Name}: {ex.Message}");
            }
        ...
        private static string ConvertOptionEntry(object? entry)
        {
            ...
            switch (entry)
            {
                ...
                case Delegate del when del.Method.GetParameters().Length == 0:
                    ...
                    {
                        TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] Delegate conversion failed: {ex.Message}");
                    }
                    break;
            }
            ...
            if (getDisplayText is not null)
            {
                ...
                {
                    TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] GetDisplayText invocation failed: {ex.Message}");
                }
            }
        ...
        private static string LookupMenu(int index)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] LookupMenu failed for {index}: {ex.Message}");
            }
        ...
        private string BuildScaleAwareLabel(string? label, bool optionIndicesChanged, int categoryId, int optionIndex, string? categoryLabel)
        {
            ...
            if (isZoom || isUiScale)
            {
                ...
                if (includeLabel)
                {
                    TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] {fixedLabel} label -> '{label ?? "<null>"}' sanitized '{sanitized}' percent {percent:0} includeLabel={includeLabel} special={special}");
                }
        ...
        private static string? DescribeFallback(int category, int option, string? categoryLabel)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[IngameSettings] Fallback description failed: {ex.Message}");
                return null;
            }
        }
