...
public sealed partial class InGameNarrationSystem
{
    private sealed partial class CraftingNarrator
    {
        ...
        /// <summary>
        /// Represents a focused recipe in the crafting UI, including position in the available recipes list.
        /// </summary>
        ...
        /// <summary>
        /// Identity of a recipe result for deduplication purposes.
        /// </summary>
        ...
        #region Recipe Focus Resolution
        ...
        private void LogMissingRequirementNarration(int recipeIndex, string label)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[CraftingNarrator] Requirement narration missing for recipe {recipeIndex} ({label})");
            }
        }

        #endregion

        #region Requirement Message Building
        ...
        private static bool TryGetRecipeBool(Recipe recipe, string[] memberNames, string debugName, out bool value)
        {
            ...
            foreach (string memberName in memberNames)
            {
                ...
                if (field is not null)
                {
                    ...
                    catch (Exception ex) when (!_loggedRecipeFlagReflectionWarning)
                    {
                        ...
                        TerrariaAccess.Instance?.Logger.Warn($"[CraftingNarrator] Failed to read recipe field {memberName}: {ex}");
                    }
                ...
                if (property is not null && property.GetIndexParameters().Length == 0 && property.PropertyType == typeof(bool))
                {
                    ...
                    {
                        _loggedRecipeFlagReflectionWarning = true;
                        TerrariaAccess.Instance?.Logger.Warn($"[CraftingNarrator] Failed to read recipe property {memberName}: {ex}");
                    }
            ...
            {
                _loggedRecipeFlagReflectionWarning = true;
                TerrariaAccess.Instance?.Logger.Debug($"[CraftingNarrator] Unable to locate recipe flag members for {debugName}");
            }
        ...
        private static string? ResolveRequirementLabel(Recipe recipe, Item ingredient, int index)
        {
            ...
            if (groupId >= 0)
            {
                ...
                catch (Exception ex) when (!_loggedRecipeGroupReflectionWarning)
                {
                    ...
                    TerrariaAccess.Instance?.Logger.Warn($"[CraftingNarrator] Failed to resolve recipe group {groupId}: {ex}");
                }
            }
        ...
        private static bool TryResolveProcessGroupLabel(Recipe recipe, Item ingredient, out string? label)
        {
            ...
            {
                _loggedRecipeGroupReflectionWarning = true;
                TerrariaAccess.Instance?.Logger.Warn($"[CraftingNarrator] Failed to process recipe group text: {ex}");
            }
        ...
        #region Recipe Group Resolution
        ...
        private static int GetAcceptedGroupId(Recipe recipe, int index)
        {
            ...
            if (AcceptedGroupResolver is not null)
            {
                ...
                {
                    _loggedRecipeGroupReflectionWarning = true;
                    TerrariaAccess.Instance?.Logger.Warn($"[CraftingNarrator] Failed to inspect recipe accepted groups: {ex}");
                }
            }
        ...
        private static Dictionary<int, int> DiscoverRecipeGroupLookup()
        {
            ...
            {
                _loggedRecipeGroupReflectionWarning = true;
                TerrariaAccess.Instance?.Logger.Warn($"[CraftingNarrator] Failed to discover recipe group lookup: {ex}");
            }
        ...
        private static Func<Recipe, int, int>? CreateAcceptedGroupResolver()
        {
            ...
            {
                _loggedRecipeGroupReflectionWarning = true;
                TerrariaAccess.Instance?.Logger.Warn($"[CraftingNarrator] Failed to bind accepted group resolver: {ex}");
            }
