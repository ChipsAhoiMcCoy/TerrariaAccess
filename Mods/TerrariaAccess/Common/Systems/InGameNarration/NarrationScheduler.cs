...
internal sealed class NarrationScheduler : INarrationScheduler
{
    ...
    {
        if (TerrariaAccess.Instance?.Logger is not { } logger)
        {
            return;
        }
...
{
    private const string TraceOnlyEnvVariable = "TerrariaAccess_SCHEDULER_TRACE_ONLY";
...
internal abstract class NarrationServiceBase : INarrationService
{
    ...
    {
        if (!context.TraceEnabled || TerrariaAccess.Instance?.Logger is not { } logger)
        {
            return;
        }
