...
public sealed class ModBrowserGamepadSystem : ModSystem
{
    ...
    public override void UpdateUI(GameTime gameTime)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Warn($"[ModBrowserGamepad] Unable to configure gamepad focus: {ex}");
        }
    }
    ...
    {
        var logger = TerrariaAccess.Instance?.Logger;
        if (logger is null)
        {
            return;
        }
    ...
    private static List<PointBinding> ApplyModRowNavigation(List<PointBinding> modRow)
    {
        ...
        TerrariaAccess.Instance?.Logger.Debug($"[ModBrowserGamepad] Mod items: {mains.Count}, actions: {actions.Count}");
