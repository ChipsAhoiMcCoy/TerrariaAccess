...
internal sealed class SettingsMenuHandler : MenuHandlerBase
{
    ...
    private bool TryHandleSettingsSlider(int currentMode, List<MenuNarrationEvent> events)
    {
        ...
        if (audioMenu)
        {
            ...
            if (isOnBackButton)
            {
                if (!State.WasOnAudioMenuBackButton)
                {
                    ...
                    TerrariaAccess.Instance?.Logger.Info($"[SettingsHandler] Audio back button: {backLabel}");
        ...
        TerrariaAccess.Instance?.Logger.Info($"[SettingsHandler] Slider {sliderId} ({kind}) -> {announcement}");
