...
internal static partial class MenuNarrationCatalog
{
    ...
    private static string TryGetFromLangMenu(int focusedIndex)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Lang.menu lookup failed (index {focusedIndex}): {ex.Message}");
        }
    ...
    public static void LogMenuSnapshot(int menuMode, bool allowRepeat = false)
    {
        ...
        TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] menuMode {menuMode} exposes {items.Length} entries.");
        for (int i = 0; i < items.Length; i++)
        {
            string entry = items[i];
            if (!string.IsNullOrWhiteSpace(entry))
            {
                TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] menuItems[{i}] = {entry}");
            }
        }
    ...
    private static string[] ConvertMenuItems(object raw)
    {
        switch (raw)
        {
            ...
            default:
                TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Unsupported menuItems type: {raw.GetType().FullName}");
                return Array.Empty<string>();
        }
    }
