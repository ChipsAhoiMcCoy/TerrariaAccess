...
internal static class ConfigElementDescriber
{
    ...
    private static string FormatConfigValue(object? value)
    {
        ...
        if (value is System.Collections.ICollection collection)
        {
            ...
            return count == 1
                ? LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.ModConfigMenu.OneItem", "1 item")
                : LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.ModConfigMenu.ItemCount", "{0} items").Replace("{0}", count.ToString());
        }
