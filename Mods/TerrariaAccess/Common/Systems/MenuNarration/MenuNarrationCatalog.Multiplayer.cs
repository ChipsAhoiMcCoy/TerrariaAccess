...
internal static partial class MenuNarrationCatalog
{
    ...
    {
        string label = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.HostMenu.LobbyLabel", "Steam lobby");
    ...
    {
        string label = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.HostMenu.FriendsLabel", "Friends can join");
        string status = DescribeEnabledDisabled(lobbyEnabled && friendsEnabled);
        if (!lobbyEnabled)
        {
            string note = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.HostMenu.RequiresLobby", "Enable the lobby to configure friend access");
    ...
    {
        string label = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.HostMenu.FriendsOfFriendsLabel", "Friends of friends can join");
        ...
        if (!friendsEnabled)
        {
            string note = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.HostMenu.RequiresFriends", "Allow friends to join before enabling this option");
            return TextSanitizer.JoinWithComma(label, status, note);
        }
    ...
    {
        string label = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.HostMenu.ConsoleLabel", "Show server console");
    ...
    private static bool MenuServerModeHasFlag(string flagName)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Failed to inspect MenuServerMode flag '{flagName}': {ex.Message}");
            return false;
        }
    }
    ...
    private static string DescribeServerIpMenu(int index)
    {
        ...
        string prompt = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.JoinMenu.ServerIpPrompt", "Enter server IP or host name");
    ...
    private static string DescribeRecentServer(int recentIndex)
    {
        ...
        string serverFormat = LocalizationHelper.GetTextOrFallback(
            "Mods.TerrariaAccess.JoinMenu.RecentServer",
    ...
    private static string DescribeServerPortMenu(int index)
    {
        ...
        // Index 2: "Accept"
        // Index 3: "Back"
        string prompt = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.JoinMenu.ServerPortPrompt", "Enter server port");
    ...
    private static string DescribeServerPasswordMenu(int index)
    {
        ...
        // Index 2: "Accept"
        // Index 3: "Back"
        string prompt = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.JoinMenu.ServerPasswordPrompt", "Enter server password");
        string password = Netplay.ServerPassword ?? string.Empty;
        string lengthFormat = LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.JoinMenu.PasswordLength", "{0} characters entered");
    ...
    {
        return LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.JoinMenu.Accept", "Accept");
    }

    private static string DescribeJoinBack()
    {
        return LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.JoinMenu.Back", "Back");
    }
