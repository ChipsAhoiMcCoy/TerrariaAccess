...
internal sealed partial class MenuUiSelectionTracker
{
    ...
    private static string DescribeCharacterCreationElement(UIElement element)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Character creation label lookup failed: {ex.Message}");
        }
    ...
    private static string DescribeDifficultyButton(UIElement root, UIElement element, bool enqueueCategoryAsPrefix = false)
    {
        ...
        if (isSelected)
        {
            parts.Add(LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.Controls.ToggleOn", "Selected"));
        }
