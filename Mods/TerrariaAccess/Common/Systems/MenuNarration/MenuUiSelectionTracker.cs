...
internal sealed partial class MenuUiSelectionTracker
{
    ...
    private static string ExtractDirectLabel(UIElement element)
    {
        ...
        if (!LoggedMissingLabels.ContainsKey(type))
        {
            ...
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Missing hover label for UI element type {type.FullName ?? "<unknown>"}.");
        }
    ...
    private static string DescribeKeybindingSimpleItem(UIElement element)
    {
        ...
        if (textFuncField?.GetValue(element) is Delegate textFunc)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Keybinding text extraction failed: {ex.Message}");
            }
        }
    ...
    private static string DescribeKeybindingToggleItem(UIElement element, bool enqueueLabelAsPrefix = true)
    {
        ...
        string state = isOn
            ? LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.Controls.ToggleOn", "On")
            : LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.Controls.ToggleOff", "Off");
    ...
    private static string InvokeFriendlyName(Type type, UIElement element)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Friendly name lookup failed for {type.Name}: {ex.Message}");
        }
    ...
    private static T InvokeFunc<T>(UIElement element, string fieldName)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Delegate '{fieldName}' invocation failed: {ex.Message}");
        }
    ...
    private static T ReadField<T>(Type type, object instance, string fieldName)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Field '{fieldName}' read failed: {ex.Message}");
        }
    ...
    private static string DescribeBindingAssignment(InputMode mode, string? keybind)
    {
        ...
        if (filtered.Count == 0)
        {
            return LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.Controls.Unbound", "Unbound");
        }
    ...
    private static void DumpElementMetadata(Type type, UIElement element)
    {
        ...
        TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Introspection for {type.FullName ?? "<unknown>"}");
        ...
        foreach (PropertyInfo property in type.GetProperties(flags))
        {
            ...
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration]  Property: {property.Name} ({typeName})");
        }

        foreach (FieldInfo field in type.GetFields(flags))
        {
            string typeName = field.FieldType.FullName ?? field.FieldType.Name;
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration]  Field: {field.Name} ({typeName})");
        }
    }
    ...
    private static string DescribeAchievementListItem(UIElement element)
    {
        ...
        string status = isCompleted
            ? LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.Achievements.Completed", "Completed")
            : LocalizationHelper.GetTextOrFallback("Mods.TerrariaAccess.Achievements.Locked", "Locked");
        parts.Add(status);
