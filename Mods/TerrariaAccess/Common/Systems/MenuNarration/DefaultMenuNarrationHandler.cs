...
/// </summary>
[System.Obsolete("Use the specialized handlers in TerrariaAccess.Common.Systems.MenuNarration.Handlers instead")]
internal sealed class DefaultMenuNarrationHandler : IMenuNarrationHandler
{
    ...
    private void HandleMenuModeChanged(MenuNarrationContext context, int currentMode, DateTime timestamp, List<MenuNarrationEvent> events)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] UI state: {context.UiState.GetType().FullName}");
        }
        else
        {
            TerrariaAccess.Instance?.Logger.Info("[MenuNarration] UI state: <null>");
        }
    ...
    private bool TryHandleUiHover(MenuNarrationContext context, DateTime timestamp, List<MenuNarrationEvent> events)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[DefaultHandler][HoverDiag] Suppressed by accessibility flag");
            return false;
        }
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[DefaultHandler][HoverDiag] Suppressed by menu mode {context.MenuMode}");
            return false;
        }
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[DefaultHandler][HoverDiag] Suppressed by UI state type: {uiStateName}");
            return false;
        }
        ...
        TerrariaAccess.Instance?.Logger.Debug($"[DefaultHandler][HoverDiag] Got hover: IsNew={hover.IsNew}, Text='{hover.Text}', menuMode={context.MenuMode}, uiState={uiStateName}");
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[DefaultHandler][HoverDiag] Suppressed hover for world creation element");
            return true;
        }
        ...
        TerrariaAccess.Instance?.Logger.Info($"[DefaultHandler][HoverDiag] *** ANNOUNCING HOVER: '{cleaned}' ***");
    ...
    private bool TryHandleSettingsSlider(int currentMode, List<MenuNarrationEvent> events)
    {
        ...
        if (audioMenu)
        {
            ...
            if (isOnBackButton)
            {
                ...
                if (!_state.WasOnAudioMenuBackButton)
                {
                    ...
                    TerrariaAccess.Instance?.Logger.Info($"[MenuNarration][AudioBack] Announcing back button: {backLabel}");
        ...
        TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] Slider {sliderId} ({kind}) -> {(sliderChanged ? $"{baseLabel}. {valueText}" : valueText)}");
    ...
    private static void LogAudioMenuDebug(int sliderIndex, string sliderLabel, MenuSliderKind kind, float percent, int categoryId, int special, string note)
    {
        try
        {
            ...
            TerrariaAccess.Instance?.Logger.Info(snapshot);
    ...
    private bool TryHandleFocus(MenuNarrationContext context, int currentMode, bool force, DateTime timestamp, List<MenuNarrationEvent> events)
    {
        ...
        if (!_focusResolver.TryGetFocus(context.Main, out MenuFocus focus))
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Unable to determine focus for menu mode {currentMode} (attempt {_state.FocusFailureCount}).");
            }
        ...
        if (shouldAnnounce)
        {
            ...
            {
                TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] Focus {focus.Index} via {focus.Source} -> {optionLabel}");
            ...
            {
                TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] Missing label for focus {focus.Index} (source {focus.Source}) in menu mode {currentMode}.");
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[MenuNarration] Focus source switched to {focus.Source} for index {focus.Index}.");
        }
    ...
    private void AnnounceFallback(MenuNarrationContext context, DateTime timestamp, List<MenuNarrationEvent> events)
    {
        ...
        TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] Fallback focus -> {fallback}");
    ...
    private bool TryHandleWorldCreationSnapshot(MenuNarrationContext context, List<MenuNarrationEvent> events)
    {
        ...
        TerrariaAccess.Instance?.Logger.Info($"[MenuNarration] World creation -> {announcement.Text}");
