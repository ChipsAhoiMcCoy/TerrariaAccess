...
public sealed class HairStyleNavigationSystem : ModSystem
{
    ...
    private static void CacheHairStyleData(UICharacterCreation self)
    {
        ...
        try
        {
            ...
            TerrariaAccess.Instance?.Logger.Debug($"[HairStyleNav] Cached {_cachedHairStyleIds.Count} hair styles");
        }
        catch (Exception ex)
        {
            TerrariaAccess.Instance?.Logger.Debug($"[HairStyleNav] Failed to cache data: {ex.Message}");
        }
    }
    ...
    private static void SelectHairStyle(int index, bool announceChange = true)
    {
        if (_cachedHairStyleIds is null || index < 0 || index >= _cachedHairStyleIds.Count)
        {
            TerrariaAccess.Instance?.Logger.Debug($"[HairStyleNav] SelectHairStyle: invalid index {index}");
            return;
        }

        if (_cachedCharacterCreation is null)
        {
            TerrariaAccess.Instance?.Logger.Debug($"[HairStyleNav] SelectHairStyle: no cached character creation");
            return;
        }
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[HairStyleNav] SelectHairStyle: player is null");
        }
    }
    ...
    private static void ScrollToIndex(int index)
    {
        ...
        {
            TerrariaAccess.Instance?.Logger.Debug($"[HairStyleNav] Failed to scroll: {ex.Message}");
        }
    }
